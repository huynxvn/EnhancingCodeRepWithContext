...
class BatchProgramClassifier(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, encode_dim, label_size, batch_size,
        ...
        # self.hidden2label = nn.Linear(self.hidden_dim * 2 * 3, self.label_size) # version history
    ...
    # def forward(self, x1, x3, x4, y1, y3, y4):
        ...
        # contexted_dis = torch.cat([abs_dist, l_code_versions, r_code_versions], 1)
...
if __name__ == '__main__':
    ...
    print("Train for clone detection - DIFF CONCAT - With CALLGRAPH ")
    ...
    for epoch in range(EPOCHS):
        ...
        while i < len(dev_data):
            ...
            dev_code_x, dev_code_versions_x, dev_calling_x, dev_called_x, dev_code_v1_x, dev_calling_v1_x, dev_called_v1_x, \
                dev_code_y, dev_code_versions_y, dev_calling_y, dev_called_y, dev_code_v1_y, dev_calling_v1_y, dev_called_v1_y, \
                    dev_labels = batch
    ...
    while i < len(test_data):
        ...
        test_code_x, test_code_versions_x, test_calling_x, test_called_x, test_code_v1_x, test_calling_v1_x, test_called_v1_x, \
                test_code_y, test_code_versions_y, test_calling_y, test_called_y, test_code_v1_y, test_calling_v1_y, test_called_v1_y, \
                    test_labels = batch
        if USE_GPU:
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-CallGraph-Clone Diff Concat, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
