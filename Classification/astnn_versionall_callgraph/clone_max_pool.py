...
class BatchProgramClassifier(nn.Module):
    ...
    # def forward(self, x1, x3, x4, y1, y3, y4):
        ...
        # lvec = lvec.view(2, self.batch_size, self.hidden_dim * 2) # version history
        # rvec = rvec.view(2, self.batch_size, self.hidden_dim * 2) # version history
        # lvec = lvec.view(3, self.batch_size, self.hidden_dim * 2) # callgraph
        # rvec = rvec.view(3, self.batch_size, self.hidden_dim * 2) # callgraph
        lvec = lvec.view(4, self.batch_size, self.hidden_dim * 2) # both
        rvec = rvec.view(4, self.batch_size, self.hidden_dim * 2) # both
...
if __name__ == '__main__':
    ...
    print("Train for clone detection - MAX POOLING - With BOTH - Version All + Call Graph ")
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-VersionAll_Callgraph-Clone Max Pool, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
