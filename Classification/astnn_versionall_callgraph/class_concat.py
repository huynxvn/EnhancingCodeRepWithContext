...
class BatchProgramClassifier(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, encode_dim, label_size, batch_size,
        ...
        self.hidden2label = nn.Linear(self.hidden_dim * 2 * 4, self.label_size) # both
    ...
    # def forward(self, x1, x3, x4):
        ...
        inputs = torch.cat([code, code_versions_all, calling, called], dim=1)
...
def get_batch(dataset, idx, bs):
    ...
    # return x1, x2, x3, x4, torch.FloatTensor(labels)
...
if __name__ == '__main__':
    ...
    print("Train for classification - CONCAT - With BOTH VerAll Callgraph")
    ...
    for epoch in range(EPOCHS):
        ...
        while i < len(train_data):
            ...
            train_code, train_code_versions, train_calling, train_called, train_code_v1, train_calling_v1, train_called_v1, train_number_of_days, train_number_of_versions, train_code_versions_all, \
                train_labels = batch
            if USE_GPU:
        ...
        while i < len(dev_data):
            ...
            dev_code, dev_code_versions, dev_calling, dev_called, dev_code_v1, dev_calling_v1, dev_called_v1, dev_number_of_days, dev_number_of_versions, dev_code_versions_all,\
                dev_labels = batch
    ...
    while i < len(test_data):
        ...
        test_code, test_code_versions, test_calling, test_called, test_code_v1, test_calling_v1, test_called_v1, test_number_of_days, test_number_of_versions, test_code_versions_all, \
            test_labels = batch
        if USE_GPU:
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-VersionAll_Callgraph-Class Concat, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
