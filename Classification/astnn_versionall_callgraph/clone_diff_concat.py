...
class BatchProgramClassifier(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, encode_dim, label_size, batch_size,
        ...
        self.hidden2label = nn.Linear(self.hidden_dim * 2 * 7, self.label_size) # both
    ...
    # def forward(self, x1, x3, x4, y1, y3, y4):
        ...
        # contexted_dis = torch.cat([abs_dist, l_code_versions, r_code_versions, l_calling, r_calling, l_called, r_called], 1)
        contexted_dis = torch.cat([abs_dist, l_code_versions_all, r_code_versions_all, l_calling, r_calling, l_called, r_called], 1)
...
if __name__ == '__main__':
    ...
    print("Train for clone detection - DIFF CONCAT - With BOTH - Version All + Call Graph ")
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-VersionAll_Callgraph-Clone Diff Concat, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
