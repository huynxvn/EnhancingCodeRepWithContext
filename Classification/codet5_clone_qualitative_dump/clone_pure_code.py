...
if __name__ == '__main__':
    ...
    print(f"Train for clone detection - {p_model} - with VersionAll+CallGraph+NumOfDays - Clone Pure Code ")
    ...
    model.load_state_dict(torch.load(MODEL_DIR + '/clone_pure_code.pth', weights_only=False))
    ...
    dump_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "predictions_baseline.json")
    ...
    model_log = "Transformer-%s-VersionAll_CallGraph_NumOfDays-Clone Pure Code, %.5f, %.5f, %.5f, %.5f, %s" % (p_model, acc, p, r, f, str_time2run)
