...
class BatchProgramClassifier(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, encode_dim, label_size, batch_size,
        ...
        self.bigru = nn.GRU(self.encode_dim, self.hidden_dim, num_layers=self.num_layers, bidirectional=True,
                            batch_first=True)
        # linear
        self.hidden2label = nn.Linear(self.hidden_dim * 2 + 1, self.label_size) # version history + number of days/versions
        # hidden
...
if __name__ == '__main__':
    ...
    print("Train for clone detection - MAX POOLING - With VERSION ALL + NUMBER OF DAYS")
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-VersionAll_NumOfDays-Clone Max Pool, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
