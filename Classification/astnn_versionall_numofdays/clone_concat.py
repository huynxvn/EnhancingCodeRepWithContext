...
p_model_save_dir = exp_setting["model_save_dir"] + "/versionall_numofdays"
if p_task == "clone":
...
class BatchProgramClassifier(nn.Module):
    ...
    # def forward(self, x1, x3, x4, y1, y3, y4):
        ...
        # breakpoint()
        lvec = torch.cat([l_code, l_code_versions_all, l_number_of_days], 1)        
        rvec = torch.cat([r_code, r_code_versions_all, r_number_of_days], 1)
...
if __name__ == '__main__':
    ...
    print("Train for clone detection - CONCATENATE - With VERSION HISTORY + NUMBER OF DAYS ")
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-VersionAll_NumOfDays-Clone Concat, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
