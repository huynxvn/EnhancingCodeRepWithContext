...
class BatchProgramClassifier(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, encode_dim, label_size, batch_size,
        ...
        self.hidden2label = nn.Linear(self.hidden_dim * 2 * 3 + 3, self.label_size) # version history
...
if __name__ == '__main__':
    ...
    print("Train for clone detection - DIFF CONCAT - With VERSION ALL + NUMBER OF DAYS ")
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-VersionAll_NumOfDays-Clone Diff Concat, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
