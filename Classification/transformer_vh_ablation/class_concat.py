...
parser.add_argument('--max_versions', nargs='?', default='all',
                    help="Number of historical versions to include: 1, 3, or 'all'", type=str)
...
# Convert max_versions to int if numeric
mv = pars['max_versions']
max_versions_val = int(mv) if mv.isdigit() else mv
...
p_model_save_dir = exp_setting["model_save_dir"] + f"/vh_ablation_{pars['max_versions']}"
if p_task == "clone":
...
if __name__ == '__main__':
    ...
    print(f"Train for classification - {p_model} - VH_Ablation_{pars['max_versions']} - CONCAT")

    model = BatchProgramClassifier(p_model, hidden_dim=768, label_size=LABELS)

    train_data = CodeClassificationDataset("train", model.tokenizer, max_versions=max_versions_val)
    val_data   = CodeClassificationDataset("dev",   model.tokenizer, max_versions=max_versions_val)
    test_data  = CodeClassificationDataset("test",  model.tokenizer, max_versions=max_versions_val)
    ...
    model_log = "Transformer-%s-VH_Ablation_%s-Class Concat, %.5f, %.5f, %.5f, %.5f, %s" % (
        p_model, pars['max_versions'], acc, p, r, f, str_time2run)
    obj = SharedFunction(model_log)
    obj.filepath = './transformer_vh_ablation/result.txt'
    obj.AppendFile()
