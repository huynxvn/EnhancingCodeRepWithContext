...
if __name__ == '__main__':
    ...
    print(f"Train for clone detection - {p_model} with VH_Ablation_{pars['max_versions']} - Clone Concat ")

    model = BatchProgramClassifier(p_model, hidden_dim=768, label_size=LABELS)

    train_data = CodeCloneDataset("train", model.tokenizer, max_versions=max_versions_val)
    val_data   = CodeCloneDataset("dev",   model.tokenizer, max_versions=max_versions_val)
    test_data  = CodeCloneDataset("test",  model.tokenizer, max_versions=max_versions_val)
    ...
    model_log = "Transformer-%s-VH_Ablation_%s-Clone Concat, %.5f, %.5f, %.5f, %.5f, %s" % (
