...
p_model_save_dir = exp_setting["model_save_dir"] + "/versionall_callgraph_numofdays"
if p_task == "clone":
...
class BatchProgramClassifier(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, encode_dim, label_size, batch_size,
        ...
        # self.hidden2label = nn.Linear(self.hidden_dim * 2 * 2 + 1, self.label_size)  # version history
        # self.hidden2label = nn.Linear(self.hidden_dim * 2 * 3, self.label_size)  # version history + number_of_days
        self.hidden2label = nn.Linear(self.hidden_dim * 2 * 4 + 1, self.label_size)  # version history all + callgraph + number_of_days
    ...
    def forward(self, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10):
        ...
        # breakpoint()
        lvec = torch.cat([l_code, l_code_versions_all, l_calling, l_called, l_number_of_days], 1)        
...
if __name__ == '__main__':
    ...
    print("Train for clone detection - CONCATENATE - With VERSION ALL + CALLGRAPH + NUM OF DAYS ")
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-VersionAll_CallGraph_NumOfDays-Clone Concat, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
