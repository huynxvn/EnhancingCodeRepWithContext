...
class BatchProgramClassifier(nn.Module):
    ...
    # def forward(self, x1, x3, x4):
        ...
        inputs = torch.cat([code, calling, called, code_versions_all], dim=0)
        ...
        inputs = inputs.view(4, self.batch_size, self.hidden_dim * 2 + 1)
...
if __name__ == '__main__':
    ...
    print("Train for classification - MAX POOL - With VERSION HISTORY ALL + CALLGRAPH + NUM OF DAYS")
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-VersionAll_CallGraph_NumOfDays-Class Max Pool, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
