...
class BatchProgramClassifier(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, encode_dim, label_size, batch_size,
        ...
        self.hidden2label = nn.Linear(self.hidden_dim * 2 * 4 + 1, self.label_size) # version history + num of days
    ...
    # def forward(self, x1, x3, x4):
        ...
        # inputs = torch.cat([code, code_versions_all, number_of_days], dim=1)
...
if __name__ == '__main__':
    ...
    print("Train for classification - CONCAT - With VERSION HISTORY ALL + CALLGRAPH + NUM OF DAYS")
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-VersionAll_CallGraph_NumOfDays-Class Concat, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
