...
def get_batch_transformer(batch):
    ...
    # return x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, y1, y2, y3, y4, y5, y6, y y8, y9, y10, train_labels
...
if __name__ == '__main__':
    ...
    print(f"Train for clone detection - {p_model} with VERSION ALL - Clone Pure Code ")
    ...
    print("Total testing results(acc,P,R,F1,time2run):%.5f, %.5f, %.5f, %.5f" % (acc, p, r, f))    

    # store model result    
    model_log = "Transformer-%s-VersionAll-Clone Pure Code, %.5f, %.5f, %.5f, %.5f, %s" % (p_model, acc, p, r, f, str_time2run)
