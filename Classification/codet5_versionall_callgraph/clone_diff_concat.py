...
class BatchProgramClassifier(nn.Module):
    def __init__(self,
        ...
        self.hidden2label = nn.Linear(self.hidden_dim * 7, self.label_size)

    def encode(self, input_ids):
        return self.encoder(input_ids,attention_mask=input_ids.ne(1))[0][:, 0,:]
    
    # def forward(self, x1=None, y1=None):
...
if __name__ == '__main__':
    ...
    print(f"Train for clone detection - {p_model} with VERSION ALL + CALL GRAPH - Clone Diff Concat")
    ...
    model = best_model
    torch.save(model.state_dict(), MODEL_DIR + '/clone_diff_concat.pth')
    ...
    model.load_state_dict(torch.load(MODEL_DIR + '/clone_diff_concat.pth'))
    ...
    model_log = "Transformer-%s-VersionAll_CallGraph-Clone Diff Concat, %.5f, %.5f, %.5f, %.5f, %s" % (p_model, acc, p, r, f, str_time2run)
