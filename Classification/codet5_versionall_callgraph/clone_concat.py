...
p_model_save_dir = exp_setting["model_save_dir"] + "/versionall_callgraph"
if p_task == "clone":
...
class BatchProgramClassifier(nn.Module):
    def __init__(self,
        ...
        self.encoder = AutoModel.from_pretrained(self.encoder_name, config = self.config, trust_remote_code=True).encoder
        ...
        self.hidden2label = nn.Linear(self.hidden_dim * 4, self.label_size)
    ...
    def forward(self, x1=None, x2=None, x3=None, x4=None, x8=None, x9=None, x10=None, y1=None, y2=None, y3=None, y4=None, y8=None, y9=None, y10=None):
        ...
        lvec = torch.cat([l_code, l_code_versions_all, l_calling, l_called], 1)
        rvec = torch.cat([r_code, r_code_versions_all, r_calling, r_called], 1)
        # lvec = torch.cat([l_code, l_calling, l_called, l_code_versions_all], 1)
        # rvec = torch.cat([r_code, r_calling, r_called, r_code_versions_all], 1)
...
def get_batch_transformer(batch):
    ...
    # return x1, x3, x4, x10, y1, y3, y4, y10, train_labels
...
if __name__ == '__main__':
    ...
    print(f"Train for clone detection - {p_model} with VERSION ALL + CALL GRAPH - Clone Concat ")
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in train_loader:
            ...
            # train_code_x, train_calling_x, train_called_x, train_code_versions_all_x, \
            #     train_code_y, train_calling_y, train_called_y, train_code_versions_all_y, \
        ...
        for batch in dev_loader:
            ...
            # dev_code_x, dev_calling_x, dev_called_x, dev_code_versions_all_x, \
            #     dev_code_y, dev_calling_y, dev_called_y, dev_code_versions_all_y, \
    ...
    for batch in test_loader:
        ...
        # test_code_x, test_calling_x, test_called_x, test_code_versions_all_x, \
        #     test_code_y, test_calling_y, test_called_y, test_code_versions_all_y, \
    ...
    model_log = "Transformer-%s-VersionAll_CallGraph-Clone ConCat, %.5f, %.5f, %.5f, %.5f, %s" % (p_model, acc, p, r, f, str_time2run)
