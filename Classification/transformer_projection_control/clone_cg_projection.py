...
p_model_save_dir = exp_setting["model_save_dir"] + "/callgraph_projection"
if p_task == "clone":
...
class BatchProgramClassifier(nn.Module):
    def __init__(self,
        ...
        self.projection = nn.Linear(self.hidden_dim * 3, self.hidden_dim)
...
if __name__ == '__main__':
    ...
    print(f"Train for clone detection - {p_model} with CALLGRAPH - Clone Projection (fixed-dim control)")
    ...
    model = best_model
    torch.save(model.state_dict(), MODEL_DIR + '/clone_cg_projection.pth')

    """
    test
    """
    model = BatchProgramClassifier(p_model, hidden_dim=768, label_size=LABELS)
    model.load_state_dict(torch.load(MODEL_DIR + '/clone_cg_projection.pth'))
    ...
    model_log = "Transformer-%s-Callgraph-Clone Projection, %.5f, %.5f, %.5f, %.5f, %s" % (p_model, acc, p, r, f, str_time2run)
    obj = SharedFunction(model_log)
    obj.AppendFile()

    print(f"[RESULT] clone_cg_projection | {pars['model']} | F1: {f:.4f} | Precision: {p:.4f} | Recall: {r:.4f}")
    ...
    results["clone_detection"][model_name]["code+cg_projection"] = {"precision": float(p), "recall": float(r), "f1": float(f)}
