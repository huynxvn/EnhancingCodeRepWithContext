...
if __name__ == '__main__':
    ...
    print(f"Train for classification - {p_model} - PROJECTION - With VERSION ALL + CALLGRAPH (fixed-dim control)")
    ...
    model = best_model
    torch.save(model.state_dict(), MODEL_DIR + '/class_vh_cg_projection.pth')
    ...
    model_log = "Transformer-%s-VersionAll_CallGraph-Class Projection, %.5f, %.5f, %.5f, %.5f, %s" % (p_model, acc, p, r, f, str_time2run)
    obj = SharedFunction(model_log)
    obj.AppendFile()

    print(f"[RESULT] class_vh_cg_projection | {pars['model']} | Accuracy: {acc:.4f} | Macro-F1: {f:.4f}")
    ...
    results["classification"][model_name]["code+vh+cg_projection"] = {"accuracy": float(acc), "macro_f1": float(f)}
