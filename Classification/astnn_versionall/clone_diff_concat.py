...
class BatchProgramClassifier(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, encode_dim, label_size, batch_size,
        ...
        self.bigru = nn.GRU(self.encode_dim, self.hidden_dim, num_layers=self.num_layers, bidirectional=True,
                            batch_first=True)
        # linear
        # self.hidden2label = nn.Linear(self.hidden_dim * 2 * 5, self.label_size) # callgraph
        self.hidden2label = nn.Linear(self.hidden_dim * 2 * 3, self.label_size) # all versions of code
    ...
    # def forward(self, x1, x3, x4, y1, y3, y4):
        ...
        # l_number_of_days, r_number_of_days = self.encode_number(x8), self.encode_number(x8)
        ...
        # l_code = torch.cat([l_code, l_number_of_days, l_number_of_vers], 1)
        # r_code = torch.cat([r_code, r_number_of_days, r_number_of_vers], 1)
        # l_code_versions = torch.cat([l_code_versions, l_number_of_days, l_number_of_vers], 1)
        # r_code_versions = torch.cat([r_code_versions, r_number_of_days, r_number_of_vers], 1)

        abs_dist = torch.abs(torch.add(l_code, -r_code))
        contexted_dis = torch.cat([abs_dist, l_code_versions_all, r_code_versions_all], 1)
...
if __name__ == '__main__':
    ...
    print("Train for clone detection - DIFF CONCAT - With ALL VERSIONS OF CODE ")
    ...
    for epoch in range(EPOCHS):
        ...
        while i < len(dev_data):
            ...
            model.hidden = model.init_hidden()
            # output = model(dev_code_x, dev_code_versions_x, dev_calling_x, dev_called_x, dev_code_y, dev_code_versions_y, dev_calling_y, dev_called_y)
            output = model(
                dev_code_x, dev_code_versions_x, dev_calling_x, dev_called_x, dev_code_v1_x, dev_calling_v1_x, dev_called_v1_x, dev_number_of_days_x, dev_number_of_versions_x, dev_code_versions_all_x, 
                dev_code_y, dev_code_versions_y, dev_calling_y, dev_called_y, dev_code_v1_y, dev_calling_v1_y, dev_called_v1_y, dev_number_of_days_y, dev_number_of_versions_y, dev_code_versions_all_y
                )
    ...
    while i < len(test_data):
        ...
        model.hidden = model.init_hidden()
        # output = model(test_code_x, test_code_versions_x, test_calling_x, test_called_x, test_code_y, test_code_versions_y, test_calling_y, test_called_y)
        output = model(
                test_code_x, test_code_versions_x, test_calling_x, test_called_x, test_code_v1_x, test_calling_v1_x, test_called_v1_x, test_number_of_days_x, test_number_of_versions_x, test_code_versions_all_x, 
                test_code_y, test_code_versions_y, test_calling_y, test_called_y, test_code_v1_y, test_calling_v1_y, test_called_v1_y, test_number_of_days_y, test_number_of_versions_y, test_code_versions_all_y
                )
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-VersionAll-Clone Diff Concat, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
