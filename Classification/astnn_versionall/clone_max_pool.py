...
class BatchProgramClassifier(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, encode_dim, label_size, batch_size,
        ...
        self.bigru = nn.GRU(self.encode_dim, self.hidden_dim, num_layers=self.num_layers, bidirectional=True,
                            batch_first=True)
        # linear
        self.hidden2label = nn.Linear(self.hidden_dim * 2, self.label_size) # version history all as string
        # hidden
    ...
    # def forward(self, x1, x3, x4, y1, y3, y4):
        ...
        # l_code_versions, r_code_version = self.encode(x2), self.encode(y2)
        ...
        lvec = torch.cat([l_code, l_code_versions_all], 0)
        rvec = torch.cat([r_code, r_code_versions_all], 0)
...
if __name__ == '__main__':
    ...
    print("Train for clone detection - MAX POOLING - With ALL VERSIONS OF CODE")
    ...
    # store model result    
    model_log = "Neural Network-ASTNN-VersionAll-Clone Max Pool, %.5f, %.5f, %.5f, %.5f, %s" % (acc, p, r, f, str_time2run)
