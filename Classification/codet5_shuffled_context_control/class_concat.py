...
p_model_save_dir = exp_setting["model_save_dir"] + "/shuffled_context_control_seed1234"
if p_task == "clone":
...
if __name__ == '__main__':
    ...
    print(f"Train for classification -{p_model}- CONCAT - With VERSIONALL + CALLGRAPH + NUMOFDAYS ")
    ...
    # store model result    
    model_log = "Transformer-%s-VersionAll_CallGraph_NumOfDays-Class Concat, %.5f, %.5f, %.5f, %.5f, %s" % (p_model, acc, p, r, f, str_time2run)
    ...
    print(f"[RESULT] shuffled_context_control | {pars['model']} | "
          f"Accuracy: {acc:.4f} | Macro-P: {p:.4f} | Macro-R: {r:.4f} | Macro-F1: {f:.4f}")

    result_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results_shuffled_control.json')
    ...
    results["classification"][model_name]["code+vh+cg+age_shuffled_seed1234"] = {
        "accuracy": float(acc),
