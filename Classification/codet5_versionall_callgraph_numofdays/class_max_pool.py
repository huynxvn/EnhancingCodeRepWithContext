...
class BatchProgramClassifier(nn.Module):
    ...
    def forward(self, x1=None, x2=None, x3=None, x4=None, x8=None, x9=None, x10=None):
        ...
        calling = torch.cat([calling, number_of_days], 1)
        called = torch.cat([called, number_of_days], 1)
        ...
        inputs = inputs.view(4, self.batch_size, self.hidden_dim + 1)
...
if __name__ == '__main__':
    ...
    print(f"Train for classification - {p_model} - MAX POOL - With VERSIONALL + CALLGRAPH + NUMOFDAYS ")
    ...
    # store model result    
    model_log = "Transformer-%s-VersionAll_CallGraph_NumOfDays-Class Max Pool, %.5f, %.5f, %.5f, %.5f, %s" % (p_model, acc, p, r, f, str_time2run)
