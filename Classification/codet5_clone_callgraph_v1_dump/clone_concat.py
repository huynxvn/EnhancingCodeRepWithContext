...
p_model_save_dir = exp_setting["model_save_dir"] + "/clone_callgraph_v1_dump"
if p_task == "clone":
...
class BatchProgramClassifier(nn.Module):
    def __init__(self,
        ...
        self.config = AutoConfig.from_pretrained(self.encoder_name, trust_remote_code=True)
        self.config.num_labels = 1

        self.encoder = AutoModel.from_pretrained(self.encoder_name, config = self.config, trust_remote_code=True, use_safetensors=True).encoder
    ...
    def forward(self, x1=None, x2=None, x3=None, x4=None, x8=None, x9=None, x10=None, y1=None, y2=None, y3=None, y4=None, y8=None, y9=None, y10=None):
        ...
        l_code_version_all, r_code_versions_all = self.encode(x10), self.encode(y10)

        # lvec = torch.cat([l_code, l_code_versions], 1)
        # rvec = torch.cat([r_code, r_code_versions], 1)
        lvec = torch.cat([l_code, l_code_version_all, l_calling, l_called, l_number_of_days], 1)
        rvec = torch.cat([r_code, r_code_versions_all, r_calling, r_called, r_number_of_days], 1)

        abs_dist = torch.abs(torch.add(lvec, -rvec)) # torch.abs(lvec-rvec)
...
if __name__ == '__main__':
    ...
    print(f"Train for clone detection - {p_model} with VersionAll+CallGraph+NumOfDays - Clone Concat ")
    ...
    model.load_state_dict(torch.load(MODEL_DIR + '/clone_concat.pth', weights_only=False))
    ...
    import pandas as pd
    import os
    raw_test_df = pd.read_pickle("./data/clone_detection/test_blocks.pkl")
    per_pair_records = []
    running_idx = 0
    ...
    for batch in test_loader:
        ...
        for idx in range(len(predicted)):
            if predicted[idx] == test_labels[idx]:
                total_acc += 1
            row = raw_test_df.iloc[running_idx]
            per_pair_records.append({
                "pair_idx": int(running_idx),
                "uid_x": int(row['id1']),
                "uid_y": int(row['id2']),
                "label": int(test_labels[idx].item()),
                "pred": int(predicted[idx].item()),
                "logit": float(output[idx].item()),
                "code_x": str(row.get('t_code_x', '')),
                "code_y": str(row.get('t_code_y', '')),
                "code_versions_x": row.get('t_code_versions_all_x', ''),
                "code_versions_y": row.get('t_code_versions_all_y', ''),
                "calling_x": str(row.get('t_calling_v1_x', '')),
                "calling_y": str(row.get('t_calling_v1_y', '')),
                "called_x": str(row.get('t_called_v1_x', '')),
                "called_y": str(row.get('t_called_v1_y', '')),
                "number_of_days_x": int(row['t_number_of_days_x'][0]),
                "number_of_days_y": int(row['t_number_of_days_y'][0]),
            })
            running_idx += 1
    ...
    dump_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "predictions_context.json")
    with open(dump_path, "w") as fp:
        json.dump(per_pair_records, fp, indent=2, default=str)
    print(f"[DUMP] wrote {len(per_pair_records)} per-pair predictions to {dump_path}")
    ...
    model_log = "Transformer-%s-VersionAll_CallGraph_NumOfDays-Clone ConCat, %.5f, %.5f, %.5f, %.5f, %s" % (p_model, acc, p, r, f, str_time2run)
