...
def train_model(model_name, bits, data_file, train_file, valid_file, test_file, 
                output_dir, use_lora=False, test_every_epoch=False, exp_type="baseline", batch_size=2, resume_from_checkpoint=None):
    ...
    train_result = trainer.train(resume_from_checkpoint=resume_from_checkpoint) # allow resuming from checkpoint if specified
...
def main():
    ...
    if args.mode in ['train', 'both']:
        ...
        train_model(args.model, args.bits, args.data_file, 
                   ...
                   exp_type=args.exp,                       # Ablation - Fixed: Pass exp_type
                   batch_size=args.batch_size,
                   resume_from_checkpoint=args.checkpoint)  # allow resuming from checkpoint if specified
        print("\n✓ Training completed!\n")
